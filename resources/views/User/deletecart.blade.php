    ...
    <dialog id="my_modal_delete{{ $key }}" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box bg-white">
            ...
            <div class="modal-action">
                ...
                <form action="{{ route('cart.delete', $item->id) }}" method="post" class="w-full">
                    @method('DELETE')
                    @csrf
                    <div>
                        <p>Apakah Anda yakin Menghapus <span class="text-red-700">{{ $item->produk->nama_Produk }}</span> ini.......?</p>
                    </div>

                    <div class="flex float-right gap-4">
                    <label for="closeModalcartdelete{{ $key }}" class="btn">Close</label>
                    <button type="submit"
                ...
                <form method="dialog" class="hidden">
                    <button class="btn" id="closeModalcartdelete{{ $key }}">Close</button>
                </form>
            </div>
        </div>
    </dialog>
