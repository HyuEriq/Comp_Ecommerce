@foreach ($slider as $key => $item)
    <dialog id="my_modal_delete{{ $key }}" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box bg-white">
            <h3 class="font-bold text-lg">Delete Slider</h3>
            <hr>
            <div class="modal-action">

                <form action="{{ route('Slider.Destroy', $item->id) }}" method="post" class="w-full">
                    @method('DELETE')
                    @csrf
                    <div>
                        <p>Apakah Anda yakin Menghapus <span class="text-red-700">{{ $item->nama_slider }}</span> ini.......?</p>
                    </div>
