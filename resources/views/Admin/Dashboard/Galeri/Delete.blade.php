@foreach ($galeri as $key => $item)
    <dialog id="my_modal_Delete{{ $key }}" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box bg-white">
            <h3 class="font-bold text-lg">Delete Galeri</h3>
            <hr>
            <div class="modal-action">

                <form action="{{ route('Galeri.Destroy', $item->id) }}" method="post" class="w-full">
                    @method('DELETE')
                    @csrf
                    <div>
                        <p>Apakah Anda yakin Menghapus <span class="text-red-700">Gambar</span> ini.......?</p>
                    </div>

                    <img src="{{ asset('storage/galeri/' . $item->gambar_galeri) }}" alt="" class="w-20 h-20 rounded-lg">

                    <div class="flex float-right gap-4">
                    <label for="closeModalGaleridelete{{ $key }}" class="btn">Close</label>
                    <button type="submit"
                ...
                <form method="dialog" class="hidden">
                    <button class="btn" id="closeModalGaleridelete{{ $key }}">Close</button>
                </form>
            </div>
        </div>
    </dialog>
