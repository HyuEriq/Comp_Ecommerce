    ...
    <dialog id="my_modal_delete{{ $key }}" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box bg-white">
            ...
            <div class="modal-action">
                ...
                <form action="{{ route('Kategory.Destroy', $item->id) }}" method="post" class="w-full">
                    @method('DELETE')
                    @csrf
                    <div>
                        <p>Apakah Anda yakin Menghapus <span class="text-red-700">{{ $item->nama_kategori }}</span> ini.......?</p>
                    </div>
