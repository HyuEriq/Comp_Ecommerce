@foreach ($dataorderan as $key => $items)
    <dialog id="my_modal_delete{{ $key }}" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box bg-white">
            <h3 class="font-bold text-lg">Delete Orderan</h3>
            <hr>
            <div class="modal-action">

                <form action="{{ route('Orderan.Destroy', $items->id) }}" method="post" class="w-full">
                    @method('DELETE')
                    @csrf
                    <div>
                        <p>Apakah Anda yakin Menghapus Orderan <span class="text-red-700">{{ $items->nama_lengkap }}</span> ini.......?</p>
                    </div>

                    <div class="flex float-right gap-4">
                    <label for="closeModalOrderandelete{{ $key }}" class="btn">Close</label>
                    <button type="submit"
                ...
                <form method="dialog" class="hidden">
                    <button class="btn" id="closeModalOrderandelete{{ $key }}">Close</button>
                </form>
            </div>
        </div>
    </dialog>
